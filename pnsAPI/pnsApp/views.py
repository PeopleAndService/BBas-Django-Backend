    ...
    if request.method == 'POST':
        ...
        if data['busRouteId'] is not None:
            ...
            if queueFilter is None : 
                ...
                for item in queueFilter:
                    if recentCheck :
                        if item.boardingCheck == 1:
                            flag = True
                            recentCheck = False
                if data['busRouteId'] is not None:
...
def api_insert():
    ...
    with open("routeInfo.json","r") as rt_json:
        json_routeInfo = json.load(rt_json)
    url = "http://openapi.tago.go.kr/openapi/service/BusRouteInfoInqireService/getRouteAcctoThrghSttnList?serviceKey=4ot5XrBL4OIP4v2fL0R9pi7TJTeuCnKCbKWoifMgtwCXcBN4qgDvCCrutWhvm0GfEOWAgVSTNOxaw0fDu2bSdg%3D%3D&_type=json"
